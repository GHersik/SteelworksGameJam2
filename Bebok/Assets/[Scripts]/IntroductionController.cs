...
public class IntroductionController : MonoBehaviour {
    ...
    public void EndStoryTelling() {
        HidePanel();
    }

    private void ShowPanel() {
        panel.DOKill();
        panel.DOFade(1, .4f).SetUpdate(true).OnComplete(SetInteractable);
    }

    private void HidePanel() {
        panel.DOKill();
        panel.DOFade(0, .4f).SetUpdate(true).OnComplete(DisableStory).OnComplete(SetNotInteractable);
    }
